...
(function () {
	...
	kazoo.init = function(params) {
		...
		function loadJsSIP() {
			kazooLoadScript(kazoo.config.paths.JsSIP, function(globalVar) {
				window.SIP = JsSIP;
				kazoo.version = 'jssip';
				params.onLoaded && params.onLoaded();
			});
		...
		if(kazoo.version === 'rtmp') {
			...
			kazoo.jssip.bindSessionEvents = function(session) {
				...
				*/
				...
				session.on('newDTMF', function(arg) {
					console.info('dtmf', arg);
				});
	...
	kazoo.connect = function(destination) {
		...
		if(kazoo.version === 'rtmp') {
			...
			console.log('Call options', callOpts);
