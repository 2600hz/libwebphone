...
(function () {
	var kazoo = {
		config: {
			paths: {
				...
				SIPml5: 'lib/dependencies/SIPml-api-1.4.217.js',
				// SIPml5: 'lib/dependencies/SIPml-api.js',
				VideoIO: 'lib/dependencies/VideoIO',
				SIPjs: 'lib/dependencies/sip-0.6.1.js'
			},
		},
		rtmp: {},
		webrtc: {},
	...
	kazoo.init = function(params) {
		...
		function loadSIPjs() {
			kazooLoadScript(kazoo.config.paths.SIPjs, function() {
				...
				kazoo.videoRemote.autoplay = "autoplay";
	...
	kazoo.register = function(params) {
		...
		params.reconnectDelay = isNaN(params.reconnectDelay) ? 5000 : (params.reconnectDelay < 1000 ? 1000 : parseInt(params.reconnectDelay));
		kazoo.params = params;

		if(kazoo.version === 'webrtc') {
			kazoo.webrtc.scheduledStop = false;
					...
					case 'terminated': {
						...
						switch(errCode) {
							...
							default:
								...
								&& e.o_event.s_phrase !== 'Request Cancelled'
								&& e.o_event.s_phrase !== 'Call Rejected') {
										kazoo.webrtc.currentCall = null;
			...
			kazoo.sipjs.userAgent.on('registrationFailed', function(arg) {
				console.info('registrationFailed', arg);
			});

			kazoo.sipjs.userAgent.on('invite', function(arg) {
				console.info('invite', arg);
			});
		...
		if(typeof params.onNotified === 'function') {
			connectivity.connectivityCallback = function(onlineHistory) {
				...
				if(notification.status === 'online' && params.reconnectMaxAttempts) { kazoo.startAutoReconnect(); }
			}
		}
	};
	...
	kazoo.logout = function() {
		if(kazoo.version === 'webrtc') {
			kazoo.webrtc.scheduledStop = true;
			kazoo.webrtc.sipStack && kazoo.webrtc.sipStack.stop();
		}
		else if(kazoo.version === 'rtmp') {
