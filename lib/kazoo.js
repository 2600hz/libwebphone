...
(function () {
    ...
    kazoo.attachMedia = function(session) {
        ...
        if ( receivers.length > 0 ) {
            var remoteStream = new global.window.MediaStream();
        ...
        if ( senders.length > 0 ) {
            var localStream = new window.MediaStream();
	...
	if(kazoo.version === 'rtmp') {
		...
		if(typeof kazoo.sipjs.session === 'undefined') {
		    ...
		    var call = {
			accept: function() {
                            var mediaEl = document.getElementById('remoteMedia');
                            var pc = kazoo.sipjs.session.DessionDescriptionHandler.peerConnection;
