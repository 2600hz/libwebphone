/*
	Kazoo WebRTC/RTMP Library v0.2.1
...
(function () {
	...
	kazoo.init = function(params) {
		...
		function loadSIPjs() {
			kazooLoadScript(kazoo.config.paths.SIPjs, function(globalVar) {
				...
				kazoo.version = 'sipjs';
				kazoo.sipjs.debug = params.debug;
	...
	kazoo.register = function(params) {
		...
		if(kazoo.version === 'rtmp') {
			...
			kazoo.jssip.userAgent.on('registrationFailed', function(arg) {
				console.info('registrationFailed', arg);
				var errCode = (arg && arg.response) ? arg.response.status_code || 0 : 0;
			...
			kazoo.jssip.userAgent.on('newRTCSession', function(arg) {
				var session = arg.session;
				console.log(kazoo.jssip.session);
				console.info('invite', session);
				if(arg.originator === 'local') {
					return;
				}
				...
				if(typeof kazoo.jssip.session === 'undefined') {
					...
					var call = {
						...
						callerName: arg.request.from.display_name,
						callerNumber: arg.request.from.uri.user
					};
			...
			kazoo.jssip.sessionEventHandlers = {
				'progress': function(arg) {
					console.info('progress', arg);
					if (arg.originator === 'remote') {
						params.onConnecting && params.onConnecting();
					}
				},
				...
				'failed': function(arg) {
					...
					if (arg.originator !== 'remote' || !arg.hasOwnProperty('message') || arg.message.status_code < 300 || arg.message.status_code > 699) {
						return;
					}
					...
					if (arg && arg.message) {
						params.onCancel({
							"code": arg.message.status_code,
							"message": arg.message.reason_phrase,
							"source": arg
						});
					} else {
						params.onCancel();
				...
				// Note: 'cancel' event not supported by JsSIP, so it is fired from hangup method, or on 'failed' event
				...
				'peerconnection': function(arg) {
					console.info('Connection created', arg);
					var peerconnection = arg.peerconnection;
				...
				'icecandidate': function(arg) {
					console.debug('Local IceCandidate gathered', arg);
					if (arg.hasOwnProperty('ready')) {
						arg.ready();
					}
				}
			};
	...
	kazoo.hangup = function() {
		if(kazoo.version === 'rtmp') {
			...
			console.info('cancel');
			kazoo.params.onCancel && kazoo.params.onCancel();
			kazoo.params.onHangup && kazoo.params.onHangup();
	...
	kazoo.transfer = function(destination) {
		if(kazoo.version === 'rtmp') {
			...
			kazoo.jssip.session.refer(destination);
		}
	...
	kazoo.connect = function(destination) {
		...
		if(kazoo.version === 'rtmp') {
			...
			kazoo.jssip.session = kazoo.jssip.userAgent.call(destination, {
	...
	kazoo.sendDTMF = function(dtmf) {
		...
		if(acceptedDTMF.indexOf(dtmf) >= 0) {
			if(kazoo.version === 'rtmp') {
				...
				kazoo.jssip.session.sendDTMF(dtmf);
			}
		}
		else {
	...
	kazoo.muteMicrophone = function(mute, success, error) {
		if(kazoo.version === 'rtmp') {
			...
			if(kazoo.jssip.hasOwnProperty('session')) {
				if(mute) {
					kazoo.jssip.session.mute();
				}
				else {
					kazoo.jssip.session.unmute();
				}
	...
	kazoo.hold = function(hold, success, error) {
		if(kazoo.version === 'sipjs') {
			if(kazoo.sipjs.hasOwnProperty('session')) {
				if(hold) {
					kazoo.sipjs.session.hold();
				}
				else {
					kazoo.sipjs.session.unhold();
				}
			...
			if(kazoo.jssip.hasOwnProperty('session')) {
				if(hold) {
					kazoo.jssip.session.hold();
				}
				else {
					kazoo.jssip.session.unhold();
				}
