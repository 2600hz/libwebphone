...
(function () {
	...
	kazoo.register = function(params) {
		...
		params.reconnectDelay = isNaN(params.reconnectDelay) ? 5000 : (params.reconnectDelay < 2000 ? 2000 : parseInt(params.reconnectDelay));
		kazoo.params = params;
	...
	kazoo.startAutoReconnect = function() {
		if(!kazoo.isReconnecting && 'params' in kazoo) {
			...
			setTimeout(function() {
				kazoo.reconnect();
				kazoo.reconnectInterval = setInterval(function() {
	...
	kazoo.stopAutoReconnect = function() {
		clearInterval(kazoo.reconnectInterval);
		kazoo.isReconnecting = false;
	};
