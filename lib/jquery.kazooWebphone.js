;(function ($, window, document) {
	...
	var namespace   = 'kazoo',
		...
		options = {
			apiUrl: 'https://sandbox.2600hz.com:8443/v2/',
			websocketUrl: 'wss://sandbox.2600hz.com:5065',
			volume: {
				range   : 'min',
				value   : 25,
				min     : 0,
		...
		widget      = {
			...
			_request: function _request (settings, callbackSuccess, callbackError) {
				...
				if (resource !== 'user_auth') {
					settings.headers['X-Auth-Token'] = authToken;
				}
			...
			_initKazoo: function _initKazoo (options) {
				var paramsInit = {
						forceRTMP: false,
				...
				if(options.hasOwnProperty('prefixScripts')) {
					paramsInit.prefixScripts = options.prefixScripts;
				}
